...
- **LLM routing role** (`llm-proxy`'s `role`: `planner` | `coder` | `coder_high` | `coder_max` | `low_cost`) is listed per agent as a required input; it is routing/cost metadata, not a prompt or tool choice. `planner`, `coder_high`, and `coder_max` all route to `claude-fable-5-1` — same per-token price as the prior `coder_high` pick (`gpt-6-astra`, $10/$50) but ahead of it on the Coding Agent Index (70 vs 67), and ahead of `claude-opus-5-5` (the prior `planner`/`coder_max` pick) too, at 2.5x Opus 5.5's price. `coder_high` is for FIXER and APP WRAPPER: they are two of the pipeline's terminal correctness gates (nothing downstream re-checks their output — FIXER is the last chance to catch a bug, APP WRAPPER is the pipeline's final stage). `coder_max` is PUBLISHER only, one tier above `coder_high` in effort (`effort: max`, vs `coder_high`'s default) — PUBLISHER, FIXER, and APP WRAPPER are the three stages that make or break the shipped app, and PUBLISHER routes to the exact same model+effort as PLANNER, via a separate role so the two can still be tuned independently later without a code change.
