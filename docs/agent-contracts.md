...
**Signing model decided: Play App Signing.** Distribution is via Google Play Console, not sideloaded/manual APKs. APP WRAPPER generates and signs with an *upload* keystore only (one-time-generated, stored as a secret); Google holds the real distribution signing key and re-signs on ingest. Lower blast radius if the upload key ever leaks (revocable/replaceable through Play Console) than a self-managed distribution key would be. Still open: whether a Play Console developer account already exists for this to publish under, and where/how the upload keystore itself gets generated and stored (Supabase secret, base64-encoded, is the default assumption pending confirmation - same pattern as every other credential this session).
