...
*GitHub App scopes are decided (Contents R&W, Pull Requests R&W, org Administration Write, Metadata Read — HANDOFF.md #6). Vercel deploy mechanism: decided — see below.*

**Implementation status: built, both halves.** No sandbox needed for either half - pure GitHub + Vercel REST API calls, since FIXER's sandbox already pushed the final commits before PUBLISHER runs. Looks up the output repo's default branch, opens a PR from `run/<run_id>` against it (idempotent - a re-run finds and reuses an existing PR instead of erroring on GitHub's 422), with a body built deterministically from the spec's function list/out-of-scope notes and FIXER's tests summary (no LLM call needed - templating from data already in hand).

**Deploy mechanism decided: direct Vercel Deployment API upload, not Vercel's GitHub Git-integration import.** Fetches the exact commit FIXER left passing file-by-file via GitHub's Git Trees/Blobs API (no sandbox, no dependency on Vercel's GitHub App having repo access), uploads directly to Vercel's `v13/deployments` endpoint, polls until `READY`, and sets `runs.web_url`. Chosen over the Git-integration path specifically to satisfy the contract's "no drift" success criterion by construction - there's no window for a later push to race a webhook-triggered rebuild. One Vercel Project per run (named from `run_id`), created implicitly on first deployment; safe to call repeatedly, no 422-style special-casing needed. Branding is additive: if `VERCEL_WILDCARD_DOMAIN` is set (a domain ASCEND owns, with a one-time wildcard DNS record already pointed at Vercel), each deployment also gets a `run-<id>.<domain>` alias; if unset or the attach call fails, falls back to Vercel's auto-assigned `*.vercel.app` URL rather than failing the run over branding. Needs `VERCEL_API_TOKEN` (required) and `VERCEL_TEAM_ID`/`VERCEL_WILDCARD_DOMAIN` (both optional) as new Supabase secrets - not yet confirmed set.

Now that `runs.web_url` is genuinely set on success, this stage reports `passed` (not always `escalated` as the PR-only build did). `escalated` is reserved for real platform-level failures: a build that fails on Vercel despite passing FIXER's own checks (a generated app that isn't a Vercel-zero-config-compatible shape - an honest, documented scope limit, not a bug), or a GitHub tree truncated because the repo's too large for one-call upload. A 403 from GitHub on PR creation still gets its own specific escalation message naming the likely cause (missing "Pull requests: Read and write" on `GITHUB_WRITE_TOKEN` - confirmed resolved).
...
**Implementation status: still blocked, not built - toolchain scoped, signing model decided.** BUILDER proved E2B sandboxes work for git/filesystem/shell work generally, but APP WRAPPER specifically needs a real Android SDK/JDK/Gradle toolchain preinstalled - E2B's default "base" template doesn't have one. Wrote the custom-template source (`supabase/functions/wrapper/sandbox-template/{e2b.Dockerfile,e2b.toml}`): JDK 17, Android cmdline-tools + platform-tools + `platforms;android-34` + `build-tools;34.0.0`, Node 20 for the Capacitor CLI. Gradle itself isn't preinstalled - every Capacitor `android/` project ships its own wrapper (`./gradlew`), which resolves its own Gradle version on first run. Not yet built/pushed to E2B's registry - this session's network policy blocks `api.e2b.dev`, so someone with E2B CLI access needs to run `e2b template build -c "black-builder-app-wrapper" -p supabase/functions/wrapper/sandbox-template` and the resulting template id gets passed as `Sandbox.create({ template: ... })`, same call shape as BUILDER/STITCHER/FIXER just naming a non-default template.

**Signing model decided: Play App Signing.** Distribution is via Google Play Console, not sideloaded/manual APKs. A Play Console developer account already exists (confirmed). APP WRAPPER generates and signs with an *upload* keystore only (one-time-generated, stored as a secret); Google holds the real distribution signing key and re-signs on ingest. Lower blast radius if the upload key ever leaks (revocable/replaceable through Play Console) than a self-managed distribution key would be. Still open: exactly where/how the upload keystore itself gets generated (a one-time local `keytool -genkeypair` run, not something this session can execute) and stored (Supabase secret, base64-encoded, is the working assumption, same pattern as every other credential this session).
...
- ~~Vercel wildcard-subdomain provisioning mechanism for PUBLISHER's web previews~~ **Decided and built:** direct Vercel Deployment API upload (zero deploy-drift), branded wildcard-subdomain aliasing layered on top as an additive `VERCEL_WILDCARD_DOMAIN` option — HANDOFF.md #4, `docs/agent-contracts.md` #6. Needs `VERCEL_API_TOKEN` (required) added as a Supabase secret before a real run can exercise it.
- ~~Signing-credential storage/provisioning for APP WRAPPER~~ **Model decided:** Play App Signing, existing Play Console dev account confirmed (mobile target itself is Capacitor — HANDOFF.md #3). Still open: generating/storing the upload keystore itself.
...
- ~~`GITHUB_WRITE_TOKEN` lacks the "Pull requests" permission~~ **Resolved:** it was scoped to Contents R&W + Administration R&W + Metadata (read) when set up for BUILDER, before PUBLISHER's PR-creation need was anticipated; "Pull requests: Read and write" has now been added.
