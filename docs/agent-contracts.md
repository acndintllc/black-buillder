...
**Implementation status: not yet built, but unblocked.** STITCHER above proves a bare E2B sandbox can run real `npm install` and act on the actual result, not just assert success — FIXER's build/lint/test job fits the identical pattern. Not yet started; needs a real STITCHER output (a manifest + green `npm install`) to test against.
