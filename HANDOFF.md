...
4. ~~Hosting/preview target for web and mobile outputs (Vercel, Netlify, self-hosted, app stores)~~ **Decided:** Vercel, for PUBLISHER's per-run web previews. Cost is a wash (both have usable free tiers), but Vercel's wildcard-subdomain/multi-tenant routing is the more mature, better-documented path for this app's exact need — one auto-provisioned URL per generated app, repeatedly, at build time. Netlify shipped an equivalent capability more recently and it's less proven for this pattern. Provisioning mechanism also decided: direct Vercel Deployment API upload (Mechanism 2) over Vercel's GitHub Git-integration import — chosen for zero deploy-drift from the exact commit FIXER verified, at the cost of more implementation work, matching the explicit call to build toward a finished "completer" now rather than defer the harder path. Branded wildcard-subdomain aliasing (`VERCEL_WILDCARD_DOMAIN`) is layered on top, additive — see `docs/agent-contracts.md` #6.
...
9. ~~Should Black Builder itself (not just its generated apps) work on mobile, web, and workstation, synced per user?~~ **Decided:** yes, using the exact same pattern already proven for APP WRAPPER's generated-app output — a Capacitor shell pointing `server.url` at the live production site (`mobile/`), not a separate mobile build with its own data. Web and workstation already worked (it's a responsive web app, usable in any browser); sync-per-user needed no new mechanism, since all real state already lives in Supabase (Postgres + Auth), not in any client — the same reasoning that makes web/mobile sync automatic for generated apps applies unchanged to Black Builder's own workspace.
